...
class IdentityTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.jwks = {"keys": [{
            "kty": "OKP", "crv": "Ed25519", "x": _b64url(oidc_public),
            "kid": "oidc-key-1", "alg": "EdDSA", "use": "sig", "key_ops": ["verify"],
        }]}
        self.config = OidcVerifierConfig(
            institution_id="bank-demo", provider_id="corp-oidc",
            issuer="https://idp.example.test", client_id="regagentops-client",
            allowed_algorithms=("EdDSA",), max_token_age_seconds=300,
        ...
        self.tool = ToolActionDescriptor(
            institution_id="bank-demo", tool_id="customer-records", action="read-summary",
        ...
        self.trust_bundle = WorkloadIdentityTrustBundle(
            institution_id="bank-demo",
            keys=(WorkloadIdentityTrustKey(
                institution_id="bank-demo", key_id="workload-key-1",
                public_key_base64url=_b64url(workload_public),
                not_before="2025-12-31T00:00:00Z", not_after="2026-12-31T00:00:00Z",
            ),),
        )
        self.signer = _Signer(self.workload_private)

    def _token(self, **overrides: object) -> str:
        claims: dict[str, object] = {
            "iss": self.config.issuer, "sub": "oidc-subject-123", "aud": self.config.client_id,
            "iat": NOW - 30, "exp": NOW + 300, "auth_time": NOW - 60,
            "nonce": "nonce-123", "acr": "urn:example:loa:2",
        }
        claims.update(overrides)
        return jwt.encode(claims, self.oidc_private, algorithm="EdDSA", headers={"kid": "oidc-key-1"})

    def _human(self):
        return verify_oidc_identity(
            self._token(), config=self.config, jwks=self.jwks, human_owner_id="owner-123",
            expected_subject="oidc-subject-123", expected_nonce="nonce-123", now_epoch=NOW,
        )

    def _signed_workload(self, **overrides: object):
        ...
        return sign_workload_identity(WorkloadIdentityStatement(**values), signer=self.signer)

    def _identity(self):
        return establish_authenticated_agent_identity(
            self.agent, human_identity=self._human(), workload_identity=self._signed_workload(),
            workload_trust_bundle=self.trust_bundle, established_at=NOW_TEXT,
        )

    def _signed_identity(self):
        return sign_authenticated_agent_identity(self._identity(), signer=self.signer)

    def _request(self) -> AgentActionEnvelope:
        return AgentActionEnvelope(
            request_id="req-identity-1", institution_id="bank-demo", agent_id="ops-assistant",
            human_owner_id="owner-123", model_provider="example-provider", model_id="example-model",
            tool_id="customer-records", action="read-summary", resource="customer/summary",
            data_classification=DataClassification.CONFIDENTIAL, business_purpose="customer-support",
            environment=Environment.TEST, risk_tier=RiskTier.MODERATE, input_digest="0" * 64,
            requested_at=NOW_TEXT,
        )

    def _policy(self) -> PolicyBundle:
        return PolicyBundle(institution_id="bank-demo", rules=(PolicyRule(
            rule_id="allow-read", institution_id="bank-demo", agent_id="ops-assistant",
            tool_id="customer-records", action="read-summary", business_purposes=("customer-support",),
            environments=(Environment.TEST,), data_classifications=(DataClassification.CONFIDENTIAL,),
            risk_tiers=(RiskTier.MODERATE,), effect=Decision.ALLOW,
        ),))

    def _engine(self) -> AuthenticatedPolicyEngine:
        return AuthenticatedPolicyEngine(AgentRegistry((self.agent,)), ToolRegistry((self.tool,)))

    def test_oidc_verification_redacts_bearer_material(self) -> None:
        raw = self._token()
    ...
    def test_oidc_rejects_wrong_subject(self) -> None:
        with self.assertRaises(OidcIdentityError):
            verify_oidc_identity(
                self._token(), config=self.config, jwks=self.jwks, human_owner_id="owner-123",
                expected_subject="different-subject", expected_nonce="nonce-123", now_epoch=NOW,
            )

    def test_oidc_rejects_expired_token(self) -> None:
        with self.assertRaises(OidcIdentityError):
            verify_oidc_identity(
                self._token(exp=NOW), config=self.config, jwks=self.jwks, human_owner_id="owner-123",
                expected_subject="oidc-subject-123", expected_nonce="nonce-123", now_epoch=NOW,
            )
    ...
    def test_oidc_rejects_remote_key_selection_header(self) -> None:
        token = jwt.encode(
            {"iss": self.config.issuer, "sub": "oidc-subject-123", "aud": self.config.client_id,
             "iat": NOW - 30, "exp": NOW + 300, "nonce": "nonce-123", "acr": "urn:example:loa:2"},
            self.oidc_private, algorithm="EdDSA", headers={"kid": "oidc-key-1", "jku": "https://attacker.invalid"},
        )
        with self.assertRaises(OidcIdentityError):
            verify_oidc_identity(
                token, config=self.config, jwks=self.jwks, human_owner_id="owner-123",
                expected_subject="oidc-subject-123", expected_nonce="nonce-123", now_epoch=NOW,
            )

    def test_workload_identity_signature_verifies(self) -> None:
        self.assertEqual(
            verify_workload_identity(self._signed_workload(), trust_bundle=self.trust_bundle, now=NOW_TEXT).agent_id,
            "ops-assistant",
        )

    def test_workload_identity_rejects_disabled_matching_key(self) -> None:
        other_private = Ed25519PrivateKey.generate()
        other_public = other_private.public_key().public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)
        bundle = WorkloadIdentityTrustBundle(institution_id="bank-demo", keys=(
            replace(self.trust_bundle.keys[0], status=TrustKeyStatus.DISABLED),
            WorkloadIdentityTrustKey(
                institution_id="bank-demo", key_id="workload-key-2", public_key_base64url=_b64url(other_public),
                not_before="2025-12-31T00:00:00Z", not_after="2026-12-31T00:00:00Z",
            ),
        ))
    ...
    def test_workload_identity_rejects_tampered_statement(self) -> None:
        ...
        with self.assertRaises(WorkloadIdentityError):
            verify_workload_identity(
                replace(signed, statement=replace(signed.statement, workload_id="different-workload")),
                trust_bundle=self.trust_bundle, now=NOW_TEXT,
            )
    ...
    def test_binding_rejects_model_identity_mismatch(self) -> None:
        with self.assertRaises(IdentityBindingError):
            establish_authenticated_agent_identity(
                self.agent, human_identity=self._human(), self_workload=None,
            )

    def test_authenticated_policy_requires_signed_context(self) -> None:
        decision = self._engine().evaluate(
            self._request(), self._policy(), self._identity(),
            identity_trust_bundle=self.trust_bundle, evaluated_at=NOW_TEXT,
        )
    ...
    def test_authenticated_policy_allows_valid_signed_context(self) -> None:
        decision = self._engine().evaluate(
            self._request(), self._policy(), self._signed_identity(),
            identity_trust_bundle=self.trust_bundle, evaluated_at=NOW_TEXT,
        )
        self.assertTrue(decision.identity_verified)
        self.assertEqual(decision.decision, Decision.ALLOW)

    def test_authenticated_policy_rejects_tampered_signed_context(self) -> None:
        signed = self._signed_identity()
        tampered = replace(signed, identity=replace(signed.identity, workload_id="forged-workload"))
        decision = self._engine().evaluate(
            self._request(), self._policy(), tampered,
            identity_trust_bundle=self.trust_bundle, evaluated_at=NOW_TEXT,
        )
        self.assertFalse(decision.identity_verified)
        self.assertEqual(decision.decision, Decision.DENY)
    ...
    def test_authenticated_policy_denies_expired_signed_context(self) -> None:
        decision = self._engine().evaluate(
            self._request(), self._policy(), self._signed_identity(),
            identity_trust_bundle=self.trust_bundle, evaluated_at="2026-01-01T00:10:00Z",
        )
        self.assertFalse(decision.identity_verified)
        self.assertEqual(decision.decision, Decision.DENY)
        self.assertIn("authenticated_identity_context_untrusted_or_expired", decision.authorization.reason_codes)

    def test_authenticated_policy_denies_after_registration_change(self) -> None:
        signed = self._signed_identity()
        ...
        decision = engine.evaluate(
            self._request(), self._policy(), signed,
            identity_trust_bundle=self.trust_bundle, evaluated_at=NOW_TEXT,
        )
        self.assertFalse(decision.identity_verified)
        self.assertEqual(decision.decision, Decision.DENY)
